...
# Debug logging for Streamlit Cloud console logs
import os
try:
    print("DEBUG - Files in directory:", os.listdir(BASE_DIR))
...
@st.cache_resource
def load_chatbot():
    """Load the lightweight scikit-learn model."""
    import pickle
...
def predict(message: str, model) -> tuple[str, float]:
    ...
    st.markdown(
        ...
        <div class="glass-card" style="border-color: rgba(239, 68, 68, 0.3); margin-bottom: 24px;">
            ...
            <p style="margin-top:12px; margin-bottom:0; font-size:0.95rem; color:#FDA4AF;">
                The AI model has not been trained yet. Please run <code>python train_lightweight.py</code> in the terminal to initialize the AI.
            </p>
        </div>
        ''',
        unsafe_allow_html=True
...
if question:
    ...
    with st.chat_message("assistant"):
        if model is None:
            answer = "Please train the AI model first by running `python train_lightweight.py` in the project folder."
        else:
            tag, confidence = predict(question, model)
            if confidence < 0.45:
